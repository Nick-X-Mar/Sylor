...
def get_offer_costings_per_offer(headers, offer_product_key):
    client, status = connect_to_dynamodb_resource()
    if status != 200:
        return func_resp(msg=client, data=[], status=status)
    ...
    if response.get('Item') is None:
        return func_resp(msg=f"offer_product with offer_product_key:{offer_product_key} not found.", data=[],
                         status=404)
    else:
...
def register_new_offer_costing(headers, offer_product, replicas=1):
    # Product_name poy prepei na mirasw tin timi
    # Ara prepei na parw ola ta products tou offer kai na ta grouparw ana product_name kai meta na dw posa sto kathe ena
    # DYNAMODB_OFFERS_PRODUCT_TABLE
    data_in = {
        "offer": "83fc8002-e1cc-4a58-9415-3a347852b3f2",
        "trip_amount": [
            "f724067d-1bd8-4b37-b4c2-8466d9763297"
        ]
    }
    ...
    if method == "GET":
        if event.get("rawPath") == '/offer_costing/offer_id':
            ...
            if offer_id is not None and offer_id != "":
                status, msg, data = get_offer_costings_per_offer(headers, offer_id)

...
def update_offer_product(headers, offer_product_key, body):
    ...
    if body.get('extra_patzoy_1_1_amount_with_charge') is not None and body.get('extra_patzoy_1_1_amount_with_charge') != "":
        if last is True:
            upEx += ","
        upEx += " extra_patzoy_1_1_amount_with_charge = :extra_patzoy_1_1_amount_with_charge"
        attValues[":extra_patzoy_1_1_amount_with_charge"] = str(body.get('extra_patzoy_1_1_amount_with_charge'))
        last = True
    if body.get('extra_patzoy_1_2_amount_with_charge') is not None and body.get('extra_patzoy_1_2_amount_with_charge') != "":
        if last is True:
            upEx += ","
        upEx += " extra_patzoy_1_2_amount_with_charge = :extra_patzoy_1_2_amount_with_charge"
        attValues[":extra_patzoy_1_2_amount_with_charge"] = str(body.get('extra_patzoy_1_2_amount_with_charge'))
        last = True
    for i in range(12):  # 0 11
        if body.get(f"extra_yalo_{i + 1}_amount_with_charge") is not None and body.get(f"extra_yalo_{i + 1}_amount_with_charge") != "":
            if last is True:
                upEx += ","
            upEx += f" extra_yalo_{i + 1}_amount_with_charge = :extra_yalo_{i + 1}_amount_with_charge"
            attValues[f":extra_yalo_{i + 1}_amount_with_charge"] = str(body.get(f'extra_yalo_{i + 1}_amount_with_charge'))
            last = True

        if body.get(f"extra_patzoy_{i + 1}_amount_with_charge") is not None and body.get(f"extra_patzoy_{i + 1}_amount_with_charge") != "":
            if last is True:
                upEx += ","
            upEx += f" extra_patzoy_{i + 1}_amount_with_charge = :extra_patzoy_{i + 1}_amount_with_charge"
            attValues[f":extra_patzoy_{i + 1}_amount_with_charge"] = str(body.get(f'extra_patzoy_{i + 1}_amount_with_charge'))
            last = True
